...
if [ "${BASH_SOURCE[0]}" != "$0" ]; then
  if [ $(complete -p | grep -c ' -F _docker docker') != 0 ]; then
    _docker_cli () {
      ...
      COMP_CWORD=$((cword+1));
      # use * instead of @ to concatenate, when assigning an array to a string
...
if [ "$ADD_UID" = "yes" ]; then
# encapsulate in order that it does not expand
  ...
  # use spaces, not commas to seperate array elements
